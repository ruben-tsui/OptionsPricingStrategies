...
# for plotting bivariate normal pdf
import numpy as np
...
class SurfaceGraph(QObject):
    ...
    def setRhoSlider(self, slider):
        if slider < 1: slider += 1
        if slider > 199: slider -= 1
        rho = (slider - 100)/100
        self.rho = rho
        rhoSlider.setValue(slider)
        label_rho.setText(f"ρ = {self.rho:.2f}")
        self.fillSqrtSinProxy()
    ...
    def enableSqrtSinModel(self, enable):
        ...
        self.m_graph.axisY().setRange(0.0, 0.25)
...
if __name__ == '__main__':
    ...
    vLayout.addWidget(label_rho)
    vLayout.addWidget(rhoSlider)
